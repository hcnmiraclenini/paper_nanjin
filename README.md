...
# 高需求高波动压力场景审计
python3 scripts/analyze_event_stress_baselines.py

# 工作日/周末场景分层现代强基线审计
python3 scripts/analyze_scene_baseline_stratification.py

# Traffic 跨粒度边界自动审计
python3 scripts/audit_traffic_boundary.py

# 审稿意见完成度自动审计
python3 scripts/audit_reviewer_completion.py
...
- 高需求高波动压力场景审计：`docs/experiments/高需求高波动压力场景审计_20260701.md`
- 场景分层现代强基线审计：`docs/experiments/场景分层现代强基线审计_20260701.md`
- Traffic 跨粒度边界自动审计：`docs/experiments/Traffic跨粒度边界自动审计_20260701.md`
- 审稿意见完成度自动审计：`docs/experiments/审稿意见完成度自动审计_20260701.md`
...
场景分层现代强基线审计显示，在测试窗口 152 个工作日和 60 个周末样本上，Strict RAMR-VE 的 MAPE/MSE/MAE 均排名第一：工作日为 18.1153%/0.038406/0.135967，周末为 19.6382%/0.029723/0.142826。该结果支持日历场景特征和场景条件化门控的误差收益，但当前测试窗口无节假日样本，因此不外推节假日场景结论。

压力场景审计显示，日总客流最高 25% 的高需求测试日期上，Strict RAMR-VE 的 MAPE/MSE/MAE 为 23.9754%/0.054716/0.187630，三项均优于五个现代强基线；日际客流绝对变化最高 25% 的日期上，Strict RAMR-VE 的 MAPE 为 24.8108%，排名第一，但 MSE/MAE 最优为 TimeMixer。该结果支持高需求压力场景优势，同时保留日际突变边界；窗口统计量只作为客流分布偏移代理，不作为外生事件因果识别。

Traffic 跨粒度边界自动审计确认，当前仓库未包含可端到端重跑的 Traffic 原始数据文件；Traffic 只能写作现有补充基准记录和结构鲁棒性边界，主结论、全部指标优势和无泄露实验均以南京南站真实双向日粒度客流为准。

审稿意见完成度自动审计覆盖两位专家共 15 条意见，检查证据文件和论文/回复关键词闭环；当前 15/15 条均通过。
